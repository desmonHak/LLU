...
from os         import getcwd, mkdir, mknod
...
from loads_files                    import print_tree, get_directory
...
if __name__ == "__main__":
    ...
    def createConfFileInit(file_path):
        print(file_path)
        mknod(file_path)

    def loadDefaultValue():
        idioma = Idiomas(parser.lenguaje) # noqa: F841

    def loadConfFile():
        if exists(os_join(getcwd(), "conf")):
            if not exists(os_join(getcwd(), "conf", "init.json")):
                ...
                # si existe cargarlo
                with open(os_join(getcwd(), "conf", "init.json")) as file:
                    try:
                ...
                idioma = Idiomas(data_conf["idioma"])  # noqa: F841

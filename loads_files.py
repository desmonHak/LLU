...
def print_tree(tree_dir, excluir_files=None):
    """_summary_
    ...
    for ruta in tree_dir.keys():
        for archivo in tree_dir[ruta]:
            ...
            if (archivo in excluir_files) == False:
                ...
                _file_ = "{}{}{}".format(ruta, _ThisSysten.splas, archivo)
                _file = open(_file_, "rb")
                hashString = md5(_file.read()).hexdigest()
                if debug: print("hash del archivo ({}): {}".format(_file_, hashString))
                _file.close()
